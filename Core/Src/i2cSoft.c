...
uint16_t TC358870_i2c_Read16(uint16_t addr)
{
	...
	// Read first byte (low byte)
	ret = i2cSoft_GetByte();
	uint8_t byte0 = ret;  // DEBUG: Track first byte
	
	// Send ACK to continue reading
	i2cSoft_Ack();
	
	// Read second byte (high byte)
	uint8_t byte1 = i2cSoft_GetByte();  // DEBUG: Track second byte
	ret = ret | (byte1 << 8);
...
uint8_t TC358870_i2c_Read32(uint16_t addr, uint32_t *data )
{
	...
	/* Read 4 bytes (little-endian): low byte first */
	uint8_t b0 = i2cSoft_GetByte();
	i2cSoft_Ack();
	uint8_t b1 = i2cSoft_GetByte();
	i2cSoft_Ack();
	uint8_t b2 = i2cSoft_GetByte();
	i2cSoft_Ack();
	uint8_t b3 = i2cSoft_GetByte();

	ret = ((uint32_t)b0) | ((uint32_t)b1 << 8) | ((uint32_t)b2 << 16) | ((uint32_t)b3 << 24);
...
static void SCLhigh ()
{
	/* Wait until SCL line actually reads high. Use logical negation
	   instead of bitwise NOT which is incorrect. Add tiny delay
	   to avoid busy tight-loop overwhelming CPU. */
	while(!(SCLread)) {
		SCLH;
		i2cSoft_Delay();
	}
...
{
	/* Simple scan of 7-bit addresses to find devices that ACK. */
	printf("I2C scan start...\r\n");
	for (uint8_t addr = 1; addr < 0x7F; addr++) {
		if (i2cSoft_DeviceExists(addr)) {
			printf("  found 0x%02X\r\n", addr);
		}
	}
